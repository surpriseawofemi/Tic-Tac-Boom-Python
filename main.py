# main.py - Enhanced main application with AI and stats tracking
import sys
import random
            ...
            if self.game_model.game_over:
                if self.game_model.winner:
                    ...
                    if random.random() < 0.3:
                        # Call the parent class's show_boom_effect method
                        super().show_boom_effect()
                        self.had_boom = True
                else:
...
def main():
    ...
    # Ask the user if they want to play with AI
    ...
    # Create game with AI and stats tracking based on user choice
    window = EnhancedTicTacBoomGame(ai_enabled=play_with_ai, ai_difficulty="medium", track_stats=True)
    window.setWindowTitle("Tic Tac Boom")
    window.show()
